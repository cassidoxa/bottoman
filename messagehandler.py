...
class MessageHandler:
    ...
    def __init__(self, msg_info, active_game, points_toggle, dbmgr):
        ...
        self.dynamic_commands = {
                'changeuser': self.change_permissions,
    ...
    def set_reminder(self, reminder_msg):
        ...
        self.dbmgr.write(
                ...
                SET config_text=? \
                WHERE config_option=?",
                (reminder_msg, 'reminder_message'))
        self.send_message("Reminder message set")
        return

    def set_reminder_interval(self, interval):
        """
        sets reminder that will send in regular intervals by time
        or number of messages to chat (specific by interval arg
        """
        ...
        if not interval:
            self.send_message(
                    f'Must use a number with this command. Will set the \
                    interval in seconds by default. Can set an interval in \
                    messages using the format "!setreminderinterval [number] \
                    messages"')
            return
        ...
        try:
            ...
            try:
                ...
                self.send_message(f'Error: Malformed command. Reminder \
        ...
        if len(interval) == 2 and interval[1] == 'messages':
            self.dbmgr.write(
                    ...
                    SET config_number=?\
                    WHERE config_option=?",
                    (interval_int, 'reminder_interval_messages',))
            self.send_message(
                    f'Reminder message set to post once every {interval_int} \
                    messages')
            ...
            self.dbmgr.write(
                    ...
                    SET config_number=? \
                    WHERE config_option=?",
                    (interval_int, 'reminder_interval_seconds'))
            ...
            self.send_message(f'Error: Malformed command. argument must be in \
                    seconds or minutes/hours with "h" or "m" after number')

        else:
            self.send_message(f'Error: Malformed command. Must in in format \
                    "!setreminderinterval [time]" or "!setreminderinterval \
                    [number] messages"')
    ...
    def add_append(self, append_str):
        """
        adds a string to database to be randomly appended
        to messages if enabled"
        """
        ...
        ids = [id[0] for id in self.dbmgr.query("SELECT id \
                                                FROM append_strings")]

        if not ids:
            self.dbmgr.write(
                    "INSERT INTO append_strings \
                    VALUES (?,?)",
                    (1, append_str.rstrip(),))
    ...
    def del_append(self, str_id):
        """
        deletes an append string by string id. intended for user to use
        !listappend to get ids before using this command/function
        """
        ...
        try:
            ...
            self.send_message(
                    f'Error deleting append string. Use the id provided by \
                    !listappend with this command')
            return

        id_list = [id[0] for id in self.dbmgr.query("SELECT id \
                                                    FROM append_strings")]
    ...
    def list_append(self):
        ...
        id_list = [id[0] for id in self.dbmgr.query(
                                            "SELECT id \
                                            FROM append_strings")]
        string_list = [string[0] for string in self.dbmgr.query(
    ...
    def set_append_cooldown(self, time):
        """
        sets append cooldown in seconds. checks if argument is
        formatted correctly if "m" or "h" is last character, use
        multiplication to set correct amount of seconds in db
        """
        ...
        try:
            ...
            try:
                ...
                self.send_message(f'Error: Malformed command. argument must be\
                        in seconds or minutes/hours with "h" or "m" after \
                        number')
                return
    ...
    # these functions pass data to the main bottoman module to be
    # sent/whispered
    ...
    def add_user(self, user_id, user_lower, user_display, init_points=1):
        self.dbmgr.write(
                ...
                VALUES (?,?,?,?,?,?)",
                (user_id, user_lower, user_display,
                    'none', init_points, self.message_time))
        return
    ...
    def check_user(self):
        """
        check if user in db. if not, retrieve twitch user id via API and add
        them or change name if there's a duplicate id. check and set user
        permissions for message being handled
        """

        user_list = [user[0] for user in self.dbmgr.query(
                                            "SELECT user_lower \
                                            FROM chatters")]

        if self.user in user_list:
            self.permissions = self.dbmgr.query(
                                "SELECT permissions \
                                FROM chatters \
                                WHERE user_lower=?",
                                (self.user,)).fetchone()[0]
            self.user_display = self.dbmgr.query(
                                "SELECT user_display \
                                FROM chatters \
                                WHERE user_lower=?",
                                (self.user,)).fetchone()[0]
            ...
            user_id_list = [id[0] for id in self.dbmgr.query(
                                                "SELECT user_id \
                                                FROM chatters")]
            ...
            if user_id not in user_id_list:
                ...
                self.dbmgr.write(
                        ...
                        user_display=? \
                        WHERE user_id=?",
                        (self.user, self.user_display, user_id,))

        return

    def add_points(self):
        """
        checks to see if points cooldown is active. if not, adds number of
        points set in db for one message the function for an admin to give a
        user a custom amount of points is give_points
        """

        old_time = self.dbmgr.query(
                    "SELECT message_time \
                    FROM chatters \
                    WHERE user_lower=?",
                    (self.user,)).fetchone()[0]
        points_cooldown = self.dbmgr.query(
                            "SELECT config_number \
                            FROM config \
                            WHERE config_option=?",
                            ('points_cooldown',)).fetchone()[0]
        points_message = self.dbmgr.query(
                            "SELECT config_number \
                            FROM config \
                            WHERE config_option=?",
                            ('points_message',)).fetchone()[0]
        ...
        if (self.message_time - old_time) >= points_cooldown:
            self.dbmgr.write(
                    ...
                    message_time=? \
                    WHERE user_lower=?",
                    (points_message, self.message_time, self.user))
    ...
    def change_permissions(self, user_new_perms):
        """
        change an existing user's permission or add user to db with
        ...
        if new_permissions not in self.permission_hierarchy.keys():
            self.send_message(
                    f'"{new_permissions}" is not a valid user \
                        permissions setting')
            return
        else:
            pass
        ...
        try:
            ...
            self.send_message(
                    f'Error adding new permissions for user "{changed_user}". \
                        Check spelling.')
            return
        ...
        if user_id in user_id_list:
            ...
            self.send_message(
                    f'{user_display} given "{new_permissions}" permissions')
    ...
    def give_points(self, user_added_points):
        ...
        try:
            ...
            self.send_message(
                    f'Error: Must use an integer number with the !give command'
                            )
            return
        ...
        try:
            user_id, user_display = self.get_user_id_display(
                                        user_added_points[0])
        except IndexError:
            self.send_message(
                    f'Error giving points: Check spelling. This command is \
                    not case-sensitive')
            return

        database_check = self.dbmgr.query(
                            "SELECT * \
                            FROM chatters \
                            WHERE user_id=?",
                            (user_id,)).fetchone()
        if database_check is None:
            self.add_user(
                    user_id, user_display.lower(),
                    user_display, added_points)
    ...
    def take_points(self, user_removed_points):
        """
        Take specified number of points from existing user
        without allowing points to go below zero
        """
        ...
        try:
            ...
            self.send_message(
                    f'Error: Must use an integer with the !take command')
            return
        ...
        try:
            user_id, user_display = self.get_user_id_display(
                                        user_removed_points[0])
        except IndexError:
            self.send_message(f'Error removing points: Check spelling. \
                    This command is not case-sensitive')
            return

        current_points = self.dbmgr.query(
                            "SELECT points \
                            FROM chatters \
                            WHERE user_id=?",
                            (user_id,)).fetchone()[0]
    ...
    def ptoggle_on(self):
        """
        turns point toggle on. When toggle is on, users will earn points for
    ...
    def ptoggle_off(self):
        """
        turns point toggle off. When toggle is off,
        users will not earn points for sending chat messages
        """
    ...
    def dynamic_command(self, command):
        """
        run commands that either have dynamic output or change some bot/user
        data self.dynamic_commands dictionary contains key-value pairs of chat
        commands and associated functions, figures out if a command needs an
        argument or not, then calls command function via dictionary dispatch
        """
    ...
    def add_command(self, added_command):
        ...
        command_list = [command[0] for command in self.dbmgr.query(
                                                    "SELECT command \
                                                    FROM commands")]
        ...
        if new_command in command_list:
            self.dbmgr.write(
                    ...
                    command_text=? \
                    WHERE command=?",
                    (new_command, command_text, new_command))
    ...
    def del_command(self, deleted_command):
        ...
        if deleted_command not in command_list:
            self.send_message(
                    f'!{deleted_command} not found in commands database')
    ...
    def list_commands(self):
        """
        List commands that users can use
        (ie not admin commands) in a chat message
        """
        ...
        command_list = [command[0] for command in self.dbmgr.query(
                                                    ...
                                                    FROM commands")]
        commands_string = ""
        reward_list = [reward[0] for reward in self.dbmgr.query(
                                                "SELECT * \
                                                FROM rewards")]
    ...
    def list_rewards(self):
        ...
        rewards_string = ""
        reward_list = [reward for reward in self.dbmgr.query(
                                                "SELECT * \
                                                FROM rewards").fetchall()]
    ...
    def add_reward(self, cost_new_reward):
        """
        add a reward to db that users can cash in points for. If reward exists,
        overwrite it. check for malformed command before accepting
        """
        ...
        try:
            ...
            self.send_message(
                    'Malformed command. Please try again in the format \
                    "!addreward [cost in points] [reward]"')
            return
    ...
    def delete_reward(self, reward):
        """
        check to see if the reward is in db then delete a reward from db and
        saves new db send message to chat about the deleted reward. function
        first manipulates the passed value to access dict key
        """
        ...
        if deleted_reward in reward_list:
            ...
            self.send_message(
                    f'Error deleting reward. Check spelling. \
    ...
    def spend_points(self, reward):
        """
        let a user spend points on a reward. if insufficient points or
        mistyped reward, send a message telling the user.if sufficient points,
        send a message saying what the user bought and send a whisper to the
        channel owner. the value passed to this function is also manipulated
        and made into a useful string in desired_reward
        """
        ...
        if desired_reward == '':
            self.send_message(
                    f'Type !rewards to see available rewards. If you have \
                    enough !points, type the reward after !buy to purchase it')
            return
        ...
        if desired_reward not in reward_list:
            self.send_message(
                    f'{self.user}, that reward does not exist. Check spelling \
                    and keep in mind that rewards are case sensitive')
            return

        elif desired_reward in reward_list:
            reward_cost = self.dbmgr.query(
                            "SELECT cost \
                            FROM rewards \
                            WHERE reward=?",
                            (desired_reward,)).fetchone()[0]

        user_points = self.dbmgr.query(
                        "SELECT points \
                        FROM chatters \
                        WHERE user_lower=?",
                        (self.user,)).fetchone()[0]
        ...
        if user_points < reward_cost:
            self.send_message(
                    f'{self.user_display}, you don\'t have enough points for \
                    "{desired_reward}"')
            return

        elif user_points >= reward_cost:
            new_points = user_points - reward_cost
            purchase_whisper = f'{self.user_display} purchased \
                                "{desired_reward}"'
    ...
    def set_points(self, points):
        ...
        if not points:
            self.send_message(
                    f'Use this command to set how many points chatters will \
                    earn per message. You can also set a points-earning \
                    cooldown with !setpointscooldown')
            return
        ...
        try:
            ...
            self.send_message(
                    f'Malformed command. Points per message must be an \
                    integer')
            return
        ...
        self.dbmgr.write(
                ...
                SET config_number=? \
                WHERE config_option=?",
                (points_per, 'points_message'))
        self.send_message(f'Points per message set to {points_per}')

    def set_points_cooldown(self, cooldown):

        """
        sets points cooldown in seconds. checks if argument is formatted
        correctly if "m" or "h" is last character, use multiplication to
        set correct amount of seconds in db
        """
        ...
        if not cooldown:
            self.send_message(
                    f'Error: Must use a number with this command. Will set \
                    the cooldown in seconds by default but you can set \
                    minutes or hours in the format "[number]m" or "[number]h"')
            return
        ...
        try:
            ...
            try:
                ...
                self.send_message(
                        f'Error: Malformed command. argument must be in \
                        seconds or minutes/hours with "h" or "m" after number')
                return
        ...
        if cooldown[0][-1].isdigit() or cooldown[0][-1] == 's':
            ...
            self.send_message(
                    f'Error: Malformed command. argument must be in seconds \
                    or minutes/hours with "h" or "m" after number')
    ...
    def handle_message(self):
        ...
        if self.message[0] == "!":
            ...
            if separate[1] in self.dynamic_commands.keys():
                ...
                command_text = self.dbmgr.query(
                                "SELECT command_text \
                                FROM commands \
                                WHERE command=?",
                                (separate[1],)).fetchone()[0]
            ...
            try:
                if (separate[1] == 'start'
                        and separate[2] in self.games_list):
                    self.bot_instructions[1]['game_instruction'] = 'start'
                    self.bot_instructions[1]['game_name'] = separate[2]

                elif separate[1] == 'stop':
                    self.bot_instructions[1]['game_instruction'] = 'stop'

                elif separate[1] == 'winner':
                    self.bot_instructions[1]['game_instruction'] = separate[2]

                elif separate[1] == 'record':
                    self.bot_instructions[1]['game_instruction'] = 'record'
